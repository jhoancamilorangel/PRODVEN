...
import { useAuth } from '../context/AuthContext'; // Asegúrate de apuntar a tu AuthContext
...
const VistaSoporteCliente = () => {
    ...
    // Encuentra el ticket de soporte existente del cliente, o crea uno
    // nuevo si es su primera vez. No requiere idEmpresa.
    useEffect(() => {
        ...
        const resolverConversacion = async () => {
            try {
                ...
                if (idResuelto) {
                    ...
                    navigate(`/soporte/${idResuelto}`, { replace: true });
                } else {
    ...
    }, [idConvUrl, idUsuarioActual, navigate]);
    ...
    if (error) {
        return (
            <div className="chat-cliente-pantalla-error">
                ...
                <p>{error}</p>
                <button onClick={() => navigate('/marketplace')} className="chat-btn-volver-error">
...
export default VistaSoporteCliente;